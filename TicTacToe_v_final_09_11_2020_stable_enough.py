        ...
        if (len(coordinates_list) == 2) or (len(coordinates_list) == 4):
            if (coordinates_list[0] in list_of_possible_coordinates_str) and (coordinates_list[1] in list_of_possible_coordinates_int):
                ...
                if coordinates_list[0] == 'a' or coordinates_list[0] == 'A':
                    ...
                    row = 2
                # function checks if given coordinates are not occupied
                if board[row][col-1] != '.':
                    coordinates_input_is_valid = False
                    os.system('cls' if os.name == 'nt' else 'clear')
                    print_board(player_mark, board)
...
def get_ai_move(board, player_mark, cpu_or_human_starts, player_switch, difficulty_level):
    free_coordinates_list = []
    # depending on chosen difficulty level, function either give random non occupied coordinates and try to prevent easy loose in easy mode, 
    # , or its trying to follow unbeatable series of moves, acting withouts mistakes and reacting for opponent 'win' opportunities in nightmare mode.
    if difficulty_level == 2:
        os.system('color 04') 
        # if player starts the game, this is path of moves cpu follows
        if cpu_or_human_starts == 'HUMAN-AI':
            for row in range(0, 3):
                for col in range(0, 3):
                    if board[row][col] == '.':
                        free_coordinates = (int(row), int(col +1))
                        free_coordinates_list.append(free_coordinates)
            # round 1
            if player_switch == 2:
            ...
            if player_switch == 8:
                ...
                if ((board[0][0] == 'X') and (board[1][0] == 'X') and (board[2][0] == '.')) or ((board[1][1] == 'X') and (board[0][2] == 'X') and (board[2][0] == '.')) or ((board[2][1] == 'X') and (board[2][2] == 'X') and (board[2][0] == '.')):
                    ...
                    coordinates_given_by_cpu = free_coordinates_list[random.randrange(len(free_coordinates_list) - 1)]
                    return coordinates_given_by_cpu
        ...
        # if cpu starts the game, this is path of moves cpu follows   
        if cpu_or_human_starts == 'AI-HUMAN':
            ...
            # round 2
            if player_switch == 3:
                # if player puts on the middle of board
                if board[1][1] == 'O':
                    if board[0][2] == '.':
                        coordinates_given_by_cpu = (0, 3)
                        return coordinates_given_by_cpu
                # if player didnt put on the middle, go either down right corner or up left corner 
                # depending of with set of two possible sets of coordinates is chosen by player  
            ...
            #round 3        
            if player_switch == 5:
                # if after player put on the middle he then put on any of corners
                if ((board[0][0] == 'O') or (board[2][2] == 'O')) and (board[1][1] == 'O'):
                    if board[0][0] == '.':
                        ...
                        coordinates_given_by_cpu = (2, 3)
                        return coordinates_given_by_cpu
                # if player doesnt block right now, cpu goes for win
                # try to win
                if ((board[1][1] == 'X') and (board[2][1] == 'X') and (board[0][1] == '.')) or ((board[0][0] == 'X') and (board[0][2] == 'X') and (board[0][1] == '.')):
                    coordinates_given_by_cpu = (0, 2)
                    return coordinates_given_by_cpu
            ...
            # round 4
            if player_switch == 7:
                # player can block, but he already loose, and cpu decides where to win depending on where player blocked
            ...
            if player_switch == 9: 
                ...
                if ((board[0][0] == 'O') and (board[1][0] == 'O') and (board[2][0] == '.')) or ((board[1][1] == 'O') and (board[0][2] == 'O') and (board[2][0] == '.')) or ((board[2][1] == 'O') and (board[2][2] == 'O') and (board[2][0] == '.')):
                    coordinates_given_by_cpu = (2, 1)
                    return coordinates_given_by_cpu
                # if nothing else is possible, cpu finds first free cell and mark it
                else:
                    coordinates_given_by_cpu = free_coordinates_list[random.randrange(len(free_coordinates_list) - 1)]
                    return coordinates_given_by_cpu
    
    # all moves 'random' apart from preventing easy loose   
        ...
        if cpu_or_human_starts == 'HUMAN-AI':
            ...
            if ((board[0][0] == 'X') and (board[1][0] == 'X') and (board[2][0] == '.')) or ((board[1][1] == 'X') and (board[0][2] == 'X') and (board[2][0] == '.')) or ((board[2][1] == 'X') and (board[2][2] == 'X') and (board[2][0] == '.')):
                coordinates_given_by_cpu = (2, 1)
                return coordinates_given_by_cpu
            # mark first free cell cpu encounter
            else:
                coordinates_given_by_cpu = free_coordinates_list[random.randrange(len(free_coordinates_list) - 1)]
                return coordinates_given_by_cpu
...
def tictactoe_game(board, player_mark, cpu_or_human_starts):
    ...
    if cpu_or_human_starts == None: 
        while game_is_over == False:
            ...
            game_is_over = perform_round_duties(coordinates, player_mark, board)
            player_switch += 1
...
def perform_round_duties(coordinates, player_mark, board):
    perform_quit(coordinates)
...
# if aftergame() value chosen by user is '2' function returns 2 and nothing happens, loop is going again to main_menu() again
main_game_loop = True
